...
{
    REQUIRE_THAT(core::str(core::detail::format_string("{} has {}.")), matchers::equal_to("{0} has {1}."));
}

TEST_CASE("format - some explicit indices", "[format]")
{
    REQUIRE_THAT(core::str(core::detail::format_string("{1} has {}.")), matchers::equal_to("{1} has {1}."));
}

TEST_CASE("format - all explicit indices", "[format]")
{
    REQUIRE_THAT(core::str(core::detail::format_string("{1} has {0}.")), matchers::equal_to("{1} has {0}."));
}

TEST_CASE("format - argument format specifiers", "[format]")
{
    REQUIRE_THAT(core::str(core::detail::format_string("{:abc} has {:def}.")), matchers::equal_to("{0:abc} has {1:def}."));
}

TEST_CASE("format - explicit indices and argument format specifiers", "[format]")
{
    REQUIRE_THAT(core::str(core::detail::format_string("{1:abc} has {0:def}.")), matchers::equal_to("{1:abc} has {0:def}."));
}
